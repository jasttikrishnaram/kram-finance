...
export async function fetchTransactions({
  month = null, limit = 500, search = '', type = '', accountId = '', categoryId = '',
  dateFrom = '', dateTo = '',
} = {}) {
  let q = supabase
    .from('transactions')
...
export async function addRecurringBill({ name, bank = '', due_day, amount, category_id = null }) {
  const { data: userData } = await supabase.auth.getUser()
  const { data, error } = await supabase
    .from('recurring_bills')
    .insert({ name, bank, due_day, amount, user_id: userData.user.id })
    .select()
    .single()
  if (error) throw error
  return data
